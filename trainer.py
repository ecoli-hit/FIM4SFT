...
import numpy
from deepspeed.utils import safe_get_full_grad, safe_set_full_fp32_param,safe_set_full_optimizer_state,safe_get_local_grad
...
class Trainer:
    ...
    def __init__(
        ...
        dump_fim: bool = False,
        dump_file: str = "/data/mxy/Finstruction/test_model/entire_FIM.pt",
        dump_thrshould: int = 200,
        ...
        self.dump_fim = dump_fim
        self.FIM = None
        self.dump_cnt = 0
        self.dump_threshould = dump_thrshould
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (
                    ...
                    # updeta FIM right after traning steps 
                    # breakpoint()
                    # print(dist.get_rank())
                    if self.dump_fim and dist.get_rank()==0:
                        if self.FIM is None:
                            self.FIM = dict()
                            print(model.local_rank)
                            for k,v in model.named_parameters():
                                if v.requires_grad:
                                    grad = safe_get_full_grad(v)
                                    tmp = torch.Tensor.cpu(grad)
                                    self.FIM[k] = tmp**2 / steps_in_epoch
                        else:
                            for k,v in model.named_parameters():
                                if v.requires_grad:
                                    grad = safe_get_full_grad(v)
            ...
            if self.dump_fim and dist.get_rank()==0 :
                path = os.path.join(self.dump_file,f"FIM_entire{dist.get_rank()}.pt")
                print(f"==== Dump Fisher Information Matrix into {path} ===")
                torch.save(self.FIM,path)
                print(self.FIM.keys())
